...
StructuredBuffer<ObjectInfo> gCbv : register(t0); // SRV: Wrapped constant buffers
...
void CSMain(uint3 groupId : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
    ...
    {
        float l = gCbv[index].boxLen;
        float3 p = gCbv[index].boxCenter;
        float3 pos[6] =
        {
            float3(p.x + l, p.y, p.z),
            float3(p.x - l, p.y, p.z),
            float3(p.x, p.y + l, p.z),
            float3(p.x, p.y - l, p.z),
            float3(p.x, p.y, p.z + l),
            float3(p.x, p.y, p.z - l)
        };
        ...
        for (int i = 0; i < 6; i++)
        {
            float4 posH = mul(float4(pos[i], 1.0f), gCbv[index].world);
            posH = mul(posH, gView);
            posH = mul(posH, gProj);
            float w = posH.w;
