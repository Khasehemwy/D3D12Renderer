...
public:
	...
	CullingApp(HINSTANCE hInstance);
...
private:
	const int mMaxInstanceNum = 8 * 200 * 20;
	int mInstanceDrawNum = 0;
...
CullingApp::CullingApp(HINSTANCE hInstance) :
	MyApp(hInstance)
{
}

bool CullingApp::Initialize()
{
...
void CullingApp::Update(const GameTimer& gt)
{
	MyApp::Update(gt);

	XMMATRIX proj = mCamera.GetProj();
	XMMATRIX viewProj = mCamera.GetView() * proj;
	...
	auto& e = mBoxGeo->DrawArgs["object"];
	for (int i = 0; i < mInstanceData.size(); i++) {

		XMMATRIX world = XMLoadFloat4x4(&mInstanceData[i].World);
		...
		if (localSpaceFrustum.Contains(e.Bounds) != DirectX::DISJOINT) {
			auto data = mInstanceData[i];
			XMStoreFloat4x4(&data.World, XMMatrixTranspose(world));
			mInstanceBuffer->CopyData(mInstanceDrawNum++, data);
		}
	}
...
void CullingApp::Draw(const GameTimer& gt)
{
...
void CullingApp::OnMouseMove(WPARAM btnState, int x, int y)
{
	MyApp::OnMouseMove(btnState, x, y);
}

void CullingApp::OnResize()
{
	MyApp::OnResize();
...
void CullingApp::BuildDescriptorHeaps()
{
...
void CullingApp::BuildRootSignature()
{
...
void CullingApp::BuildShaderAndInputLayout()
{
	HRESULT hr = S_OK;
...
void CullingApp::BuildObject()
{
		...
		for (int x = -len; x < len; x++) {
			for (int y = -len; y < len; y++) {
				for (int z = -len; z < len; z++) {
					...
					XMFLOAT3 color(r, 0, 0); r += 1.0 / mMaxInstanceNum;
	...
	BoundingBox bounds;
	float boxCenter = 0;
	XMStoreFloat3(&bounds.Center, XMLoadFloat3(&XMFLOAT3(boxCenter, boxCenter, boxCenter)));
	float boxExtents = 1;
	XMStoreFloat3(&bounds.Extents, XMLoadFloat3(&XMFLOAT3(boxExtents, boxExtents, boxExtents)));
	submesh.Bounds = bounds;

	mBoxGeo->DrawArgs["object"] = submesh;
}

void CullingApp::BuildPSO()
{
